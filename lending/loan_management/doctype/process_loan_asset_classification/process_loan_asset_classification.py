...
class ProcessLoanAssetClassification(Document):
	def on_submit(self):
		...
		update_days_past_due_in_loans(
			...
			loan_name=self.loan,
			process_loan_asset_classification=self.name,
		)
...
def create_process_loan_asset_classification(
	...
	asset_classification = frappe.new_doc("Process Loan Asset Classification")
