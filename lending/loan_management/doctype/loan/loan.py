...
def move_unpaid_interest_to_suspense_ledger(loan, posting_date=None):
	...
	if not previous_non_npa_date:
		...
		previous_non_npa_date = add_days(previous_non_npa_date, 1)
			...
			filters={
				...
				"account": (
					...
					[
						...
						accounts.additional_receivable_account,
						accounts.penalty_receivable_account,
					],
				),
				"is_cancelled": 0,
				"posting_date": ("between", [previous_non_npa_date, posting_date]),
			},
			group_by="account",
			as_list=1,
		)
	...
	if abs(amounts.get(accounts.additional_receivable_account, 0)) > 0:
