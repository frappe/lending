...
frappe.ui.form.on("Loan", {
	setup: function (frm) {
		frm.make_methods = {
			...
			"Loan Security Release": function () {
				frm.trigger("create_loan_security_release");
			},
			"Loan Write Off": function () {
	...
	onload: function (frm) {
		...
		$.each(
			["penalty_income_account", "interest_income_account"],
			function (i, field) {
		...
		$.each(
			["payment_account", "loan_account", "disbursement_account"],
			function (i, field) {
				frm.set_query(field, function () {
					return {
						filters: {
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus == 1) {
			if (
				["Disbursed", "Partially Disbursed"].includes(frm.doc.status) &&
				!frm.doc.repay_from_salary
			) {
				frm.add_custom_button(
					__("Request Loan Closure"),
					function () {
			...
			if (
				["Sanctioned", "Partially Disbursed", "Active"].includes(frm.doc.status)
			) {
				frm.add_custom_button(
			...
			if (
				["Loan Closure Requested", "Disbursed", "Partially Disbursed"].includes(
					frm.doc.status,
				)
			) {
				frm.add_custom_button(
			...
			if (
				frm.doc.status == "Loan Closure Requested" &&
				frm.doc.is_term_loan &&
				!frm.doc.is_secured_loan
			) {
				frm.add_custom_button(
					__("Close Loan"),
					function () {
			...
			args: {
				...
				disbursement_amount:
					frm.doc.loan_amount - frm.doc.disbursed_amount > 0
						? frm.doc.loan_amount - frm.doc.disbursed_amount
						: 0,
				as_dict: 1,
				repayment_frequency: frm.doc.repayment_frequency,
				is_term_loan: frm.doc.is_term_loan,
				from_foreclosed_loan: frm.doc.from_foreclosed_loan,
				amount_disbursed_in_foreclosed_loan:
					frm.doc.amount_disbursed_in_foreclosed_loan,
			},
			method:
				"lending.loan_management.doctype.loan.loan.make_loan_disbursement",
			callback: function (r) {
	...
	close_unsecured_term_loan: function (frm) {
		frappe.call({
			...
			method:
				"lending.loan_management.doctype.loan.loan.close_unsecured_term_loan",
			callback: function () {
				frm.refresh();
			},
		});
	...
	request_loan_closure: function (frm) {
		frappe.confirm(__("Do you really want to close this loan"), function () {
			frappe.call({
				args: {
					loan: frm.doc.name,
				},
				method:
					"lending.loan_management.doctype.loan.loan.request_loan_closure",
				callback: function () {
					frm.reload_doc();
				},
			});
	...
	loan_application: function (frm) {
		if (frm.doc.loan_application) {
			return frappe.call({
				method:
					"lending.loan_management.doctype.loan.loan.get_loan_application",
				args: {
